<template>
  <div class="app-container">
    ...
    <el-table :data="list" row-key="id" element-loading-text="Loading" border fit stripe highlight-current-row >
      ...
      <el-table-column label="最後修改時間" width="180">
        <template slot-scope="scope">
      ...
      <el-table-column fixed="right" label="操作" width="110">
        <template slot-scope="scope">
          <router-link :to="{ name: 'TopicEdit', params: { id: scope.row.id }}">
            <el-button type="info" icon="el-icon-edit" circle  v-if="auths.TopicEdit"></el-button>
          </router-link>
          <el-button type="danger" icon="el-icon-delete" circle  @click="funCheckDels(scope.row.id)" v-if="auths.TopicEdit"></el-button>
        </template>
      </el-table-column>
    </el-table>
...
<script>
import { getTopicList, delTopic, putTopicOnline } from '@/api/topic'
import $ from 'jquery'
export default {
  data() {
    return {
